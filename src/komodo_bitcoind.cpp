...
char *bitcoind_RPC(char **retstrp,char *debugstr,char *url,char *userpass,char *command,char *params) {
    static int didinit,count,count2;
...
int32_t notarizedtxid_height(char *dest,char *txidstr,int32_t *kmdnotarized_heightp) {
    char *jsonstr,params[256],*userpass;
    uint16_t port;
    cJSON *json,*item;
...
int32_t komodo_verifynotarization(const char *symbol,const char *dest,int32_t height,int32_t NOTARIZED_HEIGHT,uint256 NOTARIZED_HASH,uint256 NOTARIZED_DESTTXID) {
    ...
    char *hexstr;
    uint8_t *script,_script[8192];
    int32_t n,len,retval = -1;
    cJSON *json,*txjson,*vouts,*vout,*skey;
    script = _script;
...
int32_t komodo_block2height(CBlock *block) {
    ...
    int32_t i,n,height2=-1,height = 0;
    uint8_t *ptr;
...
int32_t komodo_is_special(uint8_t pubkeys[66][33],int32_t mids[66],uint32_t blocktimes[66],int32_t height,uint8_t pubkey33[33],uint32_t blocktime) {
    int32_t i,j,notaryid=0,minerid,limit,nid;
    uint8_t destpubkey33[33];
...
int8_t komodo_segid(int32_t nocache,int32_t height) {
    ...
    uint32_t txtime;
    char voutaddr[64],destaddr[64];
    int32_t txn_count,vout,newStakerActive;
    uint256 txid,merkleroot;
    CScript opret;
    int8_t segid = -1;
...
arith_uint256 komodo_PoWtarget(int32_t *percPoSp,arith_uint256 target,int32_t height,int32_t goalperc,int32_t newStakerActive) {
    ...
    CBlockIndex *pindex;
    arith_uint256 easydiff,bnTarget,hashval,sum,ave;
    bool fNegative,fOverflow;
    int32_t i,n,m,ht,percPoS,diff,val;
    *percPoSp = percPoS = 0;
...
uint32_t komodo_stake(int32_t validateflag,arith_uint256 bnTarget,int32_t nHeight,uint256 txid,int32_t vout,uint32_t blocktime,uint32_t prevtime,char *destaddr,int32_t PoSperc) {
    ...
    char address[64];
    bits256 addrhash;
    arith_uint256 hashval,mindiff,ratio,coinage256;
    uint256 hash,pasthash;
    int32_t segid,minage,i,iter=0;
    int64_t diff=0;
    uint32_t txtime,segid32,winner = 0 ;
    uint64_t value,coinage;
...
int32_t komodo_is_PoSblock(int32_t slowflag,int32_t height,CBlock *pblock,arith_uint256 bnTarget,arith_uint256 bhash) {
    ...
    int32_t ret,vout,PoSperc,txn_count,eligible=0,isPoS = 0,segid;
    uint64_t value;
        ...
        if ( myGetTransaction(KOMODO_EARLYTXID,tx,blockhash) && mapBlockIndex[blockhash]->nHeight < KOMODO_EARLYTXID_HEIGHT ) {
            for (int i = 0; i < tx.vout.size(); i++) {
                if ( tx.vout[i].scriptPubKey[0] == OP_RETURN ) {
                    ASSETCHAINS_SCRIPTPUB.pop_back();
                    ASSETCHAINS_SCRIPTPUB.pop_back(); // remove last 2 chars.
...
int64_t komodo_checkcommission(CBlock *pblock,int32_t height) {
    ...
    uint8_t *script,scripthex[8192];
    int32_t scriptlen,matched = 0;
    static bool didinit = false;
...
int32_t komodo_checkPOW(int64_t stakeTxValue, int32_t slowflag,CBlock *pblock,int32_t height) {
    ...
    int32_t i,scriptlen,possible,PoSperc,is_PoSblock=0,n,failed = 0,notaryid = -1;
...
int64_t komodo_newcoins(int64_t *zfundsp,int64_t *sproutfundsp,int32_t nHeight,CBlock *pblock) {
    CTxDestination address;
    int32_t i,j,m,n,vout;
    uint8_t *script;
    uint256 txid,hashBlock;
    int64_t zfunds=0,vinsum=0,voutsum=0,sproutfunds=0;
    n = pblock->vtx.size();
    for (i=0; i<n; i++) {
...
int64_t komodo_coinsupply(int64_t *zfundsp,int64_t *sproutfundsp,int32_t height) {
    CBlockIndex *pindex;
    CBlock block;
    int64_t zfunds=0,sproutfunds=0,supply = 0;
