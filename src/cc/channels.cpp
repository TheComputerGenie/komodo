...
CScript EncodeChannelsOpRet(uint8_t funcid,uint256 tokenid,uint256 opentxid,CPubKey srcpub,CPubKey destpub,int32_t numpayments,int64_t payment,uint256 hashchain) {
    CScript opret;
    uint8_t evalcode = EVAL_CHANNELS;
    vscript_t vopret;
...
bool ChannelsValidate(struct CCcontract_info *cp,Eval* eval,const CTransaction &tx, uint32_t nIn) {
    int32_t numvins,numvouts,preventCCvins,preventCCvouts,i,numpayments,p1,param1;
    bool retval;
    uint256 txid,hashblock,p3,param3,opentxid,tmp_txid,genhashchain,hashchain,tokenid;
    uint8_t funcid,hash[32],hashdest[32];
    char channeladdress[65],srcmarker[65],destmarker[65],destaddr[65],srcaddr[65],desttokensaddr[65],srctokensaddr[65];
...
int64_t AddChannelsInputs(struct CCcontract_info *cp,CMutableTransaction &mtx, CTransaction openTx, uint256 &prevtxid, CPubKey mypk) {
    char coinaddr[65];
    int64_t param2,totalinputs = 0,numvouts;
    uint256 txid=zeroid,tmp_txid,hashBlock,param3,tokenid;
    CTransaction tx;
    int32_t marker,param1;
...
UniValue ChannelOpen(const CPubKey& pk, uint64_t txfee,CPubKey destpub,int32_t numpayments,int64_t payment, uint256 tokenid) {
    ...
    uint64_t amount,tokens=0,funds;
    int32_t i;
    uint256 hashchain,entropy,hentropy;
    CPubKey mypk;
...
UniValue ChannelPayment(const CPubKey& pk, uint64_t txfee,uint256 opentxid,int64_t amount, uint256 secret) {
    ...
    uint256 txid,hashchain,gensecret,hashblock,entropy,hentropy,prevtxid,param3,tokenid;
...
UniValue ChannelsList(const CPubKey& pk) {
    ...
    CTransaction tx;
    char myCCaddr[65],str[512],pub[34];
    CPubKey mypk,srcpub,destpub;
    int32_t vout,numvouts,param1;
...
UniValue ChannelsInfo(const CPubKey& pk,uint256 channeltxid) {
    ...
    CTransaction tx,opentx;
    uint256 txid,tmp_txid,hashBlock,param3,opentxid,hashchain,tokenid;
    struct CCcontract_info *cp,C;
    char CCaddr[65],addr[65],str[512];
    int32_t vout,numvouts,param1,numpayments;
