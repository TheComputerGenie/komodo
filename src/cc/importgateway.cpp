...
CScript EncodeImportGatewayBindOpRet(uint8_t funcid,std::string coin,uint256 oracletxid,uint8_t M,uint8_t N,std::vector<CPubKey> importgatewaypubkeys,uint8_t taddr,uint8_t prefix,uint8_t prefix2,uint8_t wiftype) {
    CScript opret;
    uint8_t evalcode = EVAL_IMPORTGATEWAY;
...
int32_t ImportGatewayBindExists(struct CCcontract_info *cp,CPubKey importgatewaypk,std::string refcoin) {
    char markeraddr[64],burnaddr[64];
    std::string coin;
    int32_t numvouts;
    int64_t totalsupply;
...
bool ImportGatewayValidate(struct CCcontract_info *cp,Eval *eval,const CTransaction &tx, uint32_t nIn) {
    ...
    std::vector<uint8_t> proof;
    int64_t amount,tmpamount;
    uint256 hashblock,txid,bindtxid,deposittxid,withdrawtxid,completetxid,tmptokenid,oracletxid,bindtokenid,burntxid,tmptxid,merkleroot,mhash;
    CTransaction bindtx,tmptx;
    std::string refcoin,tmprefcoin,hex,name,description,format;
    CPubKey pubkey,tmppubkey,importgatewaypk;
...
std::string ImportGatewayDeposit(uint64_t txfee,uint256 bindtxid,int32_t height,std::string refcoin,uint256 burntxid,int32_t claimvout,std::string rawburntx,std::vector<uint8_t>proof,CPubKey destpub,int64_t amount) {
    ...
    CTransaction bindtx;
    CPubKey mypk;
    uint256 oracletxid,merkleroot,mhash,hashBlock,txid;
...
std::string ImportGatewayWithdraw(uint64_t txfee,uint256 bindtxid,std::string refcoin,CPubKey withdrawpub,int64_t amount) {
    CMutableTransaction mtx = CreateNewContextualCMutableTransaction(Params().GetConsensus(), komodo_nextheight());
    CTransaction tx;
    CPubKey mypk,importgatewaypk,signerpk;
    uint256 txid,hashBlock,oracletxid,tmptokenid,tmpbindtxid,withdrawtxid;
    int32_t vout,numvouts;
    int64_t nValue,inputs,CCchange=0,tmpamount;
    uint8_t funcid,K,M,N,taddr,prefix,prefix2,wiftype;
    std::string coin,hex;
    std::vector<CPubKey> msigpubkeys;
    char burnaddr[64],str[65],coinaddr[64];
    struct CCcontract_info *cp,C;
...
std::string ImportGatewayPartialSign(uint64_t txfee,uint256 lasttxid,std::string refcoin, std::string hex) {
    ...
    std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> > unspentOutputs;
    char funcid,str[65],burnaddr[64];
    int32_t numvouts;
    uint256 withdrawtxid,hashBlock,bindtxid,tokenid,oracletxid;
...
std::string ImportGatewayCompleteSigning(uint64_t txfee,uint256 lasttxid,std::string refcoin,std::string hex) {
    ...
    std::string coin,tmphex;
    CTransaction tx,tmptx;
    uint256 withdrawtxid,hashBlock,tokenid,bindtxid,oracletxid;
    int32_t numvouts;
    uint8_t K=0,M,N,taddr,prefix,prefix2,wiftype;
    std::vector<CPubKey> pubkeys;
...
std::string ImportGatewayMarkDone(uint64_t txfee,uint256 completetxid,std::string refcoin) {
    ...
    CPubKey mypk;
    struct CCcontract_info *cp,C;
    char str[65],burnaddr[64];
    CTransaction tx;
    int32_t numvouts;
    uint256 withdrawtxid,bindtxid,oracletxid,tokenid,hashBlock;
    std::string coin,hex;
...
UniValue ImportGatewayPendingWithdraws(uint256 bindtxid,std::string refcoin) {
    ...
    std::vector<CPubKey> msigpubkeys;
    uint256 hashBlock,txid,tmpbindtxid,tmptokenid,oracletxid,withdrawtxid;
    uint8_t K,M,N,taddr,prefix,prefix2,wiftype;
...
UniValue ImportGatewayProcessedWithdraws(uint256 bindtxid,std::string refcoin) {
    ...
    CTransaction tx;
    std::string coin,hex;
    CPubKey mypk,importgatewaypk,withdrawpub;
    std::vector<CPubKey> msigpubkeys;
    uint256 withdrawtxid,hashBlock,txid,tmptokenid,oracletxid;
    uint8_t K,M,N,taddr,prefix,prefix2,wiftype;
...
UniValue ImportGatewayList() {
    ...
    std::vector<uint256> txids;
    struct CCcontract_info *cp,C;
    uint256 txid,hashBlock,oracletxid;
    CTransaction vintx;
    std::string coin;
    char str[65],burnaddr[64];
    uint8_t M,N,taddr,prefix,prefix2,wiftype;
...
UniValue ImportGatewayExternalAddress(uint256 bindtxid,CPubKey pubkey) {
    ...
    std::string coin;
    int64_t numvouts;
    char str[65],addr[65],burnaddr[65];
    uint8_t M,N,taddr,prefix,prefix2,wiftype;
    std::vector<CPubKey> msigpubkeys;
...
UniValue ImportGatewayInfo(uint256 bindtxid) {
    ...
    std::string coin;
    char str[67],numstr[65],burnaddr[64],gatewaystokens[64];
    uint8_t M,N;
    std::vector<CPubKey> pubkeys;
    uint8_t taddr,prefix,prefix2,wiftype;
    uint256 oracletxid,hashBlock;
    CTransaction tx;
    CPubKey ImportGatewaypk;
    struct CCcontract_info *cp,C;
    int32_t i;
    int64_t numvouts,remaining;
    std::vector<CPubKey> msigpubkeys;
