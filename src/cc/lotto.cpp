...
UniValue LottoInfo(uint256 lottoid) {
    ...
    uint256 hashBlock,hentropy;
    CTransaction vintx;
    uint64_t lockedfunds,sbits;
    int32_t ticketsize,odds,firstheight,period;
    CPubKey lottopk;
...
UniValue LottoList() {
    ...
    struct CCcontract_info *cp,C;
    uint256 txid,hashBlock,hentropy;
    CTransaction vintx;
    uint64_t sbits;
    int32_t ticketsize,odds,firstheight,period;
    char str[65];
...
std::string LottoCreate(uint64_t txfee,char *planstr,int64_t funding,int32_t ticketsize,int32_t odds,int32_t firstheight,int32_t period) {
    ...
    CPubKey mypk,lottopk;
    uint64_t sbits;
    int64_t inputs,CCchange=0,nValue=COIN;
    struct CCcontract_info *cp,C;
    cp = CCinit(&C,EVAL_LOTTO);
    if ( txfee == 0 )
        txfee = 10000;
