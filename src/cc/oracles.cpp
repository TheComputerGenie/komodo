...
int64_t OracleCurrentDatafee(uint256 reforacletxid,char *markeraddr,CPubKey publisher) {
    ...
    int32_t dheight=0,vout,height,numvouts;
    CTransaction tx;
    CPubKey pk;
...
uint256 OraclesBatontxid(uint256 reforacletxid,CPubKey refpk) {
    ...
    CPubKey pk;
    int32_t numvouts,height,maxheight=0;
    int64_t datafee;
...
int32_t oracle_format(uint256 *hashp,int64_t *valp,char *str,uint8_t fmt,uint8_t *data,int32_t offset,int32_t datalen) {
    char _str[65];
    int32_t sflag = 0,i,val32,len = 0,slen = 0,dlen = 0;
    uint32_t uval32;
    uint16_t uval16;
    int16_t val16;
    int64_t val = 0;
        ...
        if ( len == 32 ) {
            ...
            if ( sflag != 0 ) {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    iguana_rwnum(0,&data[offset],len,(void *)&val16);
                    val = val16;
                    break;
                case 4:
                    iguana_rwnum(0,&data[offset],len,(void *)&val32);
                    val = val32;
                    break;
                case 8:
                    iguana_rwnum(0,&data[offset],len,(void *)&val);
                    break;
                }
                if ( str != 0 )
                ...
                *valp = val;
                ...
                case 1:
                    ...
                    break;
                case 2:
                    iguana_rwnum(0,&data[offset],len,(void *)&uval16);
                    uval = uval16;
                    break;
                case 4:
                    iguana_rwnum(0,&data[offset],len,(void *)&uval32);
                    uval = uval32;
                    break;
                case 8:
                    iguana_rwnum(0,&data[offset],len,(void *)&uval);
                    break;
                }
                if ( str != 0 )
...
int64_t _correlate_price(int64_t *prices,int32_t n,int64_t price) {
    int32_t i,count = 0;
    int64_t diff,threshold = (price >> 8);
    for (i=0; i<n; i++) {
...
int64_t AddMyOraclesFunds(struct CCcontract_info *cp,CMutableTransaction &mtx,CPubKey pk,uint256 oracletxid) {
    ...
    int32_t numvouts,vout,ignorevin;
    std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> > unspentOutputs;
    CTransaction vintx;
    CPubKey tmppk;
...
UniValue OracleFormat(uint8_t *data,int32_t datalen,char *format,int32_t formatlen) {
    ...
    int32_t i,j=0;
    int64_t val;
    char str[IGUANA_MAXSCRIPTSIZE*2+1];
...
UniValue OracleInfo(uint256 origtxid) {
    ...
    char str[67],markeraddr[64],numstr[64],batonaddr[64];
    std::vector <uint8_t> data;
