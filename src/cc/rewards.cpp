...
int64_t AddRewardsInputs(CScript &scriptPubKey,uint64_t maxseconds,struct CCcontract_info *cp,CMutableTransaction &mtx,CPubKey pk,int64_t total,int32_t maxinputs,uint64_t refsbits,uint256 reffundingtxid) {
    ...
    uint256 txid,hashBlock,fundingtxid;
    CTransaction tx;
    int32_t numblocks,j,vout,n = 0;
    uint8_t funcid;
...
UniValue RewardsList() {
    ...
    uint256 txid,hashBlock;
    CTransaction vintx;
    uint64_t sbits,APR,minseconds,maxseconds,mindeposit;
    char str[65];
    cp = CCinit(&C,EVAL_REWARDS);
...
std::string RewardsLock(uint64_t txfee,char *planstr,uint256 fundingtxid,int64_t deposit) {
    ...
    CPubKey mypk,rewardspk;
    CScript opret;
    uint64_t lockedfunds,sbits,funding,APR,minseconds,maxseconds,mindeposit;
    struct CCcontract_info *cp,C;
    if ( deposit < txfee ) {
...
std::string RewardsUnlock(uint64_t txfee,char *planstr,uint256 fundingtxid,uint256 locktxid) {
    ...
    char coinaddr[64];
    CPubKey mypk,rewardspk;
    CScript scriptPubKey,ignore;
    uint256 hashBlock;
    uint64_t sbits,APR,minseconds,maxseconds,mindeposit;
    int64_t funding,reward=0,amount=0,inputs,CCchange=0;
    struct CCcontract_info *cp,C;
    cp = CCinit(&C,EVAL_REWARDS);
    if ( txfee == 0 )
        txfee = 10000;
