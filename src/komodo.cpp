...
int32_t komodo_voutupdate(bool fJustCheck,int32_t *isratificationp,int32_t notaryid,uint8_t *scriptbuf,
    ...
    if ( scriptbuf[len++] == 0x6a ) {
        ...
        struct komodo_ccdataMoMoM MoMoMdata;
...
void adjust_hwmheight(int32_t newHeight) {
    hwmheight = newHeight;
}
void clear_fp_stateupdate() {
    fp = nullptr;    // tests should clear fp, before new call(s) to komodo_stateupdate if datadir is changed
}

int32_t komodo_connectblock(bool fJustCheck, CBlockIndex *pindex,CBlock& block) {
    int32_t staked_era;
    static int32_t lastStakedEra;
    std::vector<int32_t> notarisations;
    uint64_t signedmask,voutmask;
    char symbol[KOMODO_ASSETCHAIN_MAXLEN],dest[KOMODO_ASSETCHAIN_MAXLEN];
    struct komodo_state *sp;
    uint8_t scriptbuf[10001],pubkeys[64][33],rmd160[20],scriptPubKey[35];
    uint256 zero,btctxid,txhash;
