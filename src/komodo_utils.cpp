...
void OS_randombytes(unsigned char *x,long xlen) {
    ...
    while (xlen > 0) {
        if (xlen < 1048576) i = (int32_t)xlen;
        else i = 1048576;
...
void komodo_configfile(const char *symbol,uint16_t rpcport) {
    ...
    uint16_t kmdport;
    uint8_t buf2[33];
    char fname[512],buf[128],username[512],password[8192];
    uint32_t crc,r,r2,i;
...
uint16_t komodo_userpass(char *userpass,const char *symbol) {
    FILE *fp;
    uint16_t port = 0;
    char fname[512],username[512],password[512],confname[KOMODO_ASSETCHAIN_MAXLEN];
    userpass[0] = 0;
