...
UniValue height_MoM(const UniValue& params, bool fHelp, const CPubKey& mypk) {
    int32_t height,depth,notarized_height,MoMoMdepth,MoMoMoffset,kmdstarti,kmdendi;
    uint256 MoM,MoMoM,kmdtxid;
...
UniValue importdual(const UniValue& params, bool fHelp, const CPubKey& mypk) {
    ...
    std::string hex,source,sourceaddr,destaddr,burntxid;
    uint64_t burnAmount;
    CPubKey destpub;
    std::vector<CTxOut> vouts;
...
UniValue getNotarisationsForBlock(const UniValue& params, bool fHelp, const CPubKey& mypk) {
    ...
    int8_t numNN = 0, numSN = 0;
    uint8_t notarypubkeys[64][33] = {0};
    uint8_t LABSpubkeys[64][33] = {0};
    ...
    for (CWallet::TxItems::reverse_iterator it = txOrdered.rbegin(); it != txOrdered.rend(); ++it) {
        ...
        if (pwtx != 0) {
            ...
            if (pwtx->vout.size() > 0 && GetOpReturnData(pwtx->vout.back().scriptPubKey, vopret) && !vopret.empty() &&
                ...
                if (std::find_if(targetSymbol.begin(), targetSymbol.end(), [](int c) {
