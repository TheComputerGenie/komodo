...
int32_t komodo_check_deposit(int32_t height,const CBlock& block) {
    ...
    if ( chainName.isKMD() ||
        ...
        int64_t val,prevtotal = 0;
        int32_t strangeout=0,overflow = 0;
        uint64_t total = 0;
...
void komodo_stateind_set(struct komodo_state *sp,uint32_t *inds,int32_t n,uint8_t *filedata,long datalen,const char *symbol,const char *dest) {
    uint8_t func;
    long lastK,lastT,lastN,lastV,fpos,lastfpos;
    int32_t i,count,doissue,iter,numn,numv,numN,numV,numR;
    uint32_t tmp,prevpos100,offset;
